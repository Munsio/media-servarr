...
- **config** - General config data (where the sqlite database lives), backed by a PersistentVolumeClaim named `sonarr-config`
- **downloads** - Downloads folder for monitoring (plain `emptyDir` by default)
- **tv** - Location of TV (plain `emptyDir` by default)
...
Metrics are served on port `9706`.
...
**Your existing data is safe.** The `config` PersistentVolumeClaim keeps its exact original name (`sonarr-config`) by default, so a normal `helm upgrade` re-adopts the same PVC and bound volume without recreating it — no manual steps needed for a stock install.
...
**Your data is unaffected.** The `sonarr-config` PVC keeps its exact name and is mounted the same way — this change only affects how the pod is managed, not storage. Since Deployment and StatefulSet are different Kubernetes resource kinds, `helm upgrade` will delete the old Deployment and create a new StatefulSet, causing one extra pod restart during this specific upgrade — no different in effect from any routine version bump.
